				...
				<div class="row align-items-end">
					...
					<div class="col-md-2">
						<div class="form-group">
							...
							<input type="text" id="amount" placeholder="Enter number"  class="form-control" pattern="^\d+(\.\d{1,2})?$" />
						</div>
					</div>
...
var item_arr = $.parseJSON('<?php echo json_encode($item_arr) ?>');
	...
	function calc_total(){
		...
		$('[name="sub_total"]').val(parseFloat(total).toFixed(2))
		$('#sub_total').text(parseFloat(total).toFixed(2).toLocaleString('en-US'))
		...
		$('#gtotal').text(parseFloat(gtotal).toFixed(2).toLocaleString('en-US'))
	...
	function add_item($obj = null){
		...
		td = $("<td>")
			...
			td.text(parseFloat(price).toFixed(2).toLocaleString('en-US'))
			tr.append(td)
		// total column
		td = $("<td>")
			td.addClass('text-right')
			td.text(parseFloat(total).toFixed(2).toLocaleString('en-US'))
			tr.append(td)
		// action column
		td = $("<td>")
			td.addClass('text-center')
